...
export function tonMetaWalletConfigToCell(config: TonMetaWalletConfig): Cell {
  ...
  return beginCell()
    ...
    .storeAddress(config.withdrawDelegate)
...
export class TonMetaWallet implements Contract {
  ...
  async sendCreateUser(
    ...
    await provider.internal(via, {
      value: opts.value,
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: beginCell().storeUint(OpCode.createUser, 32).storeInt(opts.userId, 64).storeUint(opts.bump, 8).storeAddress(opts.userWalletAddress).endCell(),
    });
  }
  ...
  async sendNativeDeposit(
    ...
    await provider.internal(via, {
      value: opts.value,
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: beginCell().storeUint(OpCode.nativeDeposit, 32).storeUint(opts.queryId, 64).storeBuffer(opts.receiver).storeCoins(opts.amount).endCell(),
    });
  }
  ...
  async sendUserNativeWithdrawByDelegate(
    provider: ContractProvider,
    via: Sender,
    opts: {
      userId: bigint;
    ...
    await provider.internal(via, {
      ...
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: beginCell()
        .storeUint(OpCode.userNativeWithdraw, 32)
        .storeUint(opts.nonce, 128)
        .storeCoins(opts.amount)
        .storeRef(signatureCell)
        .storeUint(opts.userId, 64)
        .storeAddress(opts.receiver)
        .endCell(),
    });
  }

  async sendUserTokenWithdrawByDelegate(
    provider: ContractProvider,
    via: Sender,
    opts: {
      userId: bigint;
  ...
  async sendChangeWithdrawDelegate(
    provider: ContractProvider,
    via: Sender,
    opts: {
      value: bigint;
    ...
    await provider.internal(via, {
      value: opts.value,
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: beginCell().storeUint(OpCode.changeWithdrawDelegate, 32).storeAddress(opts.newDelegate).endCell(),
    });
  }
  ...
  async getDepositJettonAddress(provider: ContractProvider, nonce: bigint): Promise<Address> {
    const result = await provider.get("get_deposit_jetton_address", [
      {
        type: "int",
        value: nonce,
      } as TupleItemInt,
  ...
  async getWithdrawDelegate(provider: ContractProvider): Promise<Address> {
    const result = await provider.get("get_withdraw_delegate", []);
  ...
  async getMetaWalletData(provider: ContractProvider) {
    ...
    const withdrawDelegate = res.stack.readAddress();
